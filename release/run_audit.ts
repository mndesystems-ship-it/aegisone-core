...
import { APP_ENTRY_PATHS, DEFAULT_AUDIT_OUTPUT_DIR, NODE_BINARY_PATH, PACKAGE_ROOT, RUST_PARITY_BINARY_PATH } from "./paths.ts";
import { formatProvenanceForDisplay, readProvenance } from "./provenance.ts";
...
function runChecked(command: string, args: string[], cwd = PACKAGE_ROOT, extraEnv?: NodeJS.ProcessEnv): string {
...
function main() {
  ...
  runChecked(NODE_BINARY_PATH, [APP_ENTRY_PATHS.emitParityVectors]);
  const rustParityStdout = runChecked(RUST_PARITY_BINARY_PATH, [parityVectorPath]);
  writeFileSync(rustParityOutputPath, rustParityStdout, "utf8");

  runChecked(NODE_BINARY_PATH, [APP_ENTRY_PATHS.auditRun], PACKAGE_ROOT, {
    RUST_PARITY_OUTPUT_PATH: rustParityOutputPath
  });
  runChecked(NODE_BINARY_PATH, [APP_ENTRY_PATHS.attackWave]);
  runChecked(NODE_BINARY_PATH, [APP_ENTRY_PATHS.remediationWave]);
  runChecked(NODE_BINARY_PATH, [APP_ENTRY_PATHS.postRemediationVerification]);
  runChecked(NODE_BINARY_PATH, [APP_ENTRY_PATHS.controlledBenchmark]);
  runChecked(NODE_BINARY_PATH, [APP_ENTRY_PATHS.sustainedBenchmark, "--duration-seconds", String(args.benchmark_duration_seconds)]);
  ...
  writeJson(path.join(args.output_dir, "audit_runner_summary.json"), summary as unknown as JsonValue);
  process.stdout.write(`${formatProvenanceForDisplay(provenance)}\n`);
  process.stdout.write(`${canonicalizeJson(summary as unknown as JsonValue)}\n`);
}
