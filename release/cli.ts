...
import { assertCustodyOnlyBehavior, runCustodyRuntime, runPreflightCheck } from "../custody/runtime.ts";
import { verifyCustodyReceipt } from "../shared/custody_keys.js";
import { assertReleaseIntegrity, readPublishedHash } from "./integrity.ts";
...
function printVerificationResult(): void {
...
async function main() {
  ...
  if (command === "preflight-check") {
    const result = runPreflightCheck({ configPath: parseFlag(argv, "--config") ?? undefined });
  ...
  if (command === "custody") {
    runCustodyRuntime({ once: hasFlag(argv, "--check-startup"), configPath: parseFlag(argv, "--config") ?? undefined });
    return;
  }
...
void main().catch((error) => {
  process.stderr.write(`${JSON.stringify({ verdict: "REFUSE", error: error.message, code: error.code ?? "ERR_RUNTIME_ERROR" })}\n`);
  process.exit(1);
});
