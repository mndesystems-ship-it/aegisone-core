...
import { buildBenchmarkMatrix } from "../audit/harness.ts";
import { executeDeterministicPipeline, resetRuntimeState } from "../audit/node_runtime.ts";
import { DEFAULT_BENCHMARK_OUTPUT_DIR } from "./paths.ts";
...
function main() {
  ...
  const allLatencies = windows.flatMap((window) => [window.latency_ms.p50, window.latency_ms.p95, window.latency_ms.p99, window.latency_ms.p999]);
  const summary = {
    ...
    success_criteria: {
      ...
      no_percentile_above_2ms: windows.every(
        (window) =>
          window.latency_ms.p50 <= 2 &&
          window.latency_ms.p95 <= 2 &&
          window.latency_ms.p99 <= 2 &&
          window.latency_ms.p999 <= 2
      )
    }
  };

  writeJson(path.join(args.output_dir, "sustained_benchmark_report.json"), summary as unknown as JsonValue);
