...
import { canonicalizeJson, hashCanonicalJson, type JsonValue } from "../shared/index.ts";
...
type CostDecision = {
  decision: Decision;
  reasons: string[];
  projected_total_cost_micro_usd: number;
...
type AgentPolicy = {
  ...
  max_depth_of_subtasks: number;
  allowed_tool_list: string[];
  forbidden_actions: string[];
  max_total_cost_micro_usd: number;
};
...
type AgentDecision = {
  ...
  decision_hash: string;
  latency_microseconds: number;
  contains_loop_risk: boolean;
  contains_unsafe_action: boolean;
};

type ReplayEntry =
  | { kind: "cost"; input: CostRequest; expected_hash: string; expected_decision: Decision }
  | { kind: "agent"; input: AgentRun; expected_hash: string; expected_decision: Decision };
...
const agentPolicy: AgentPolicy = {
  ...
  forbidden_actions: ["delete_file", "shell_exec_destructive", "exfiltrate_secret", "modify_policy", "network_unapproved"],
...
function latencySummaryNanoseconds(values: number[]) {
  const sorted = [...values].sort((left, right) => left - right);
...
function distributionBuckets(values: number[]) {
  const buckets = [
    { upper_bound_ns: 50_000, count: 0 },
    { upper_bound_ns: 100_000, count: 0 },
    { upper_bound_ns: 250_000, count: 0 },
    { upper_bound_ns: 500_000, count: 0 },
    { upper_bound_ns: 1_000_000, count: 0 },
    { upper_bound_ns: 2_000_000, count: 0 },
    { upper_bound_ns: 5_000_000, count: 0 },
    { upper_bound_ns: -1, count: 0 }
  ];
...
function measureDecisionLatencySamples<TInput>(
  label: string,
  inputs: TInput[],
  sampleTarget: number,
  evaluate: (input: TInput) => unknown
) {
...
type AnchorWorkItem =
  | { kind: "cost"; input: CostRequest }
  | { kind: "agent"; input: AgentRun };

function buildMixed5050Anchor(costRequests: CostRequest[], agentRuns: AgentRun[]): AnchorWorkItem[] {
  ...
  const anchor: AnchorWorkItem[] = [];

  for (let index = 0; index < 125; index += 1) {
    anchor.push({ kind: "cost", input: allowCost[index]! });
    anchor.push({ kind: "cost", input: refuseCost[index]! });
    anchor.push({ kind: "agent", input: allowAgent[index]! });
    anchor.push({ kind: "agent", input: refuseAgent[index]! });
  }

  return anchor;
}

function evaluateAnchorWorkItem(item: AnchorWorkItem) {
  if (item.kind === "cost") {
...
function measureAnchorMixed5050(anchor: AnchorWorkItem[]) {
...
export function generateCostRequests(count: number): CostRequest[] {
    ...
    if (category === "normal") {
      ...
      request = {
        ...
        requested_runtime_seconds:
          subtype === 1 ? costPolicy.max_runtime_seconds : rng.nextInt(4_200, costPolicy.max_runtime_seconds),
        requested_retry_count:
          subtype === 2 ? costPolicy.max_retry_count : rng.nextInt(Math.max(0, costPolicy.max_retry_count - 1), costPolicy.max_retry_count),
...
function evaluateCostRequest(request: CostRequest, policy: CostPolicy): CostDecision {
  ...
  if (request.requested_runtime_seconds > policy.max_runtime_seconds) {
    reasons.push(
      `requested_runtime_seconds=${request.requested_runtime_seconds} exceeds max_runtime_seconds=${policy.max_runtime_seconds}`
    );
  }
  ...
  const latencyMicroseconds =
    82 +
    request.requested_gpu_count * 5 +
    Math.floor(request.requested_runtime_seconds / 180) +
    request.requested_retry_count * 11 +
    request.autoscale_target * 13 +
    request.agent_loop_iterations * 17;
...
export function generateAgentRuns(count: number): AgentRun[] {
  const runs: AgentRun[] = [];
  for (let index = 0; index < count; index += 1) {
...
function evaluateAgentRun(run: AgentRun, policy: AgentPolicy): AgentDecision {
  const reasons: string[] = [];
  ...
  if (run.max_depth_requested > policy.max_depth_of_subtasks) {
    reasons.push(
      `max_depth_requested=${run.max_depth_requested} exceeds max_depth_of_subtasks=${policy.max_depth_of_subtasks}`
    );
  }
  ...
  const latencyMicroseconds =
    94 +
    run.steps_planned * 4 +
    run.max_depth_requested * 19 +
    run.actions.length * 7 +
    (containsLoopRisk ? 37 : 0) +
    (containsUnsafeAction ? 29 : 0);
...
function runCostBenchmark(requests: CostRequest[]) {
  ...
  const categoryBreakdown = { normal: 0, borderline: 0, runaway: 0 };
  for (const request of requests) {
...
function runAgentBenchmark(runs: AgentRun[]) {
  const latencyValues: number[] = [];
  let runsCompleted = 0;
  let runsStopped = 0;
...
function runDeterminismCheck(costRequest: CostRequest, agentRun: AgentRun) {
...
function runReplayVerification(entries: ReplayEntry[]) {
  let mismatches = 0;
  ...
  for (let iteration = 0; iteration < REPLAY_ITERATIONS; iteration += 1) {
    ...
    const current =
      entry.kind === "cost"
        ? evaluateCostRequest(entry.input, costPolicy)
        : evaluateAgentRun(entry.input, agentPolicy);
...
async function writeJsonArtifact(outputDir: string, fileName: string, value: JsonValue) {
...
async function main() {
  ...
  const mixed5050AnchorWorkload = mixed5050Anchor.map((item) =>
    item.kind === "cost"
      ? { kind: "cost", id: item.input.request_id, expected_decision: item.input.expected_decision }
      : { kind: "agent", id: item.input.run_id, expected_decision: item.input.expected_decision }
  );
  const workloadManifest = {
    ...
    anchor_test: {
      ...
      workload_hash: sha256(stableJson(mixed5050AnchorWorkload as unknown as JsonValue))
    },
    measurement_policy: {
  ...
  const replayReport = runReplayVerification([...costReport.replay_entries, ...agentReport.replay_entries]);
  const anchorReference = measureAnchorMixed5050(mixed5050Anchor);
  const costRealLatency = measureDecisionLatencySamples(
    "cost_control",
    costRequests,
    COST_REAL_LATENCY_SAMPLE_TARGET,
    (request) => evaluateCostRequest(request, costPolicy)
  );
  const agentRealLatency = measureDecisionLatencySamples(
    "agent_control",
    agentRuns,
    AGENT_REAL_LATENCY_SAMPLE_TARGET,
    (run) => evaluateAgentRun(run, agentPolicy)
  );
  const combinedRealLatencySamples = [...costRealLatency.raw_samples_ns, ...agentRealLatency.raw_samples_ns];
  ...
  const latencyReport = {
    ...
    simulated_latency: {
      ...
      combined_decision_layer: latencySummary(
        [...costLatencies, ...agentLatencies].filter((value) => value > 0)
      )
    },
    real_latency: {
  ...
  const latencyRealValidation = {
    ...
    outlier_count: {
      ...
      combined_decision_layer: combinedRealLatencySamples.reduce(
        (count, sample) =>
          count +
          (sample >
          Math.max(latencyReport.real_latency.combined_decision_layer.p99 * 2, 1_000_000)
            ? 1
            : 0),
        0
      )
  ...
  const {
    replay_entries: costReplayEntries,
    ...costArtifactBody
  } = costReport;
  const {
    replay_entries: agentReplayEntries,
    ...agentArtifactBody
  } = agentReport;
  ...
  await writeJsonArtifact(outputDir, "summary.json", summary as JsonValue);
  await writeJsonArtifact(outputDir, "cost_report.json", costArtifact as JsonValue);
  await writeJsonArtifact(outputDir, "agent_behavior_report.json", agentArtifact as JsonValue);
  await writeJsonArtifact(outputDir, "determinism_report.json", determinismReport as JsonValue);
  await writeJsonArtifact(outputDir, "replay_report.json", replayReport as JsonValue);
  await writeJsonArtifact(outputDir, "latency_report.json", latencyReport as JsonValue);
  await writeJsonArtifact(outputDir, "latency_real_validation.json", latencyRealValidation as JsonValue);
  await writeJsonArtifact(outputDir, "workload_manifest.json", workloadManifest as JsonValue);

  const manifest = {
    benchmark_version: BENCHMARK_VERSION,
    output_dir: OUTPUT_DIR_NAME,
    artifacts: [
      { file: "summary.json", sha256: sha256(`${stableJson(summary as JsonValue)}\n`) },
      { file: "cost_report.json", sha256: sha256(`${stableJson(costArtifact as JsonValue)}\n`) },
      { file: "agent_behavior_report.json", sha256: sha256(`${stableJson(agentArtifact as JsonValue)}\n`) },
      { file: "determinism_report.json", sha256: sha256(`${stableJson(determinismReport as JsonValue)}\n`) },
      { file: "replay_report.json", sha256: sha256(`${stableJson(replayReport as JsonValue)}\n`) },
      { file: "latency_report.json", sha256: sha256(`${stableJson(latencyReport as JsonValue)}\n`) },
      { file: "latency_real_validation.json", sha256: sha256(`${stableJson(latencyRealValidation as JsonValue)}\n`) },
      { file: "workload_manifest.json", sha256: sha256(`${stableJson(workloadManifest as JsonValue)}\n`) }
    ]
  };

  await writeJsonArtifact(outputDir, "manifest.json", manifest as JsonValue);
