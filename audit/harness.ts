...
export type BenchmarkCase = {
  ...
  expected_decision: "ALLOW" | "REFUSE";
  request_size_bytes: number;
  nested_depth: number;
  high_cardinality_count: number;
  key_order_seed: number;
  tags: string[];
};
...
function makeToken(rng: Rng, prefix: string, targetLength: number): string {
...
function makeSizeTarget(caseIndex: number): number {
...
function buildToolCalls(count: number): Array<{ tool: string; priority: number }> {
  const output: Array<{ tool: string; priority: number }> = [];
...
function cloneInput(input: CanonicalExecutionInput): CanonicalExecutionInput {
  return JSON.parse(JSON.stringify(input)) as CanonicalExecutionInput;
}

function stableSize(value: JsonValue): number {
  return Buffer.byteLength(JSON.stringify(value), "utf8");
}

function reorderJson(value: JsonValue, rng: Rng): JsonValue {
...
function serializeJson(value: JsonValue, rng: Rng, whitespace = 0): string {
  return JSON.stringify(reorderJson(value, rng), null, whitespace);
}

function computeDepth(value: JsonValue): number {
  if (Array.isArray(value)) {
...
function inflateRequest(
  ...
  let currentSize = stableSize(next as unknown as JsonValue);
  ...
  currentSize = stableSize(next as unknown as JsonValue);
  if (currentSize >= targetBytes) {
    return next;
  }
...
function makeValidCase(profile: Exclude<ProfileId, "adversarial_malformed">, index: number): BenchmarkCase {
  ...
  const rawValue = reorderJson(input as unknown as JsonValue, createRng(seed ^ 0xabcddcba));
  ...
  return {
    ...
    expected_valid: true,
    expected_decision:
      profile === "allow_burst" || profile === "replay_storm" || (profile === "mixed_50_50" && index % 2 === 1)
        ? "ALLOW"
        : "REFUSE",
...
function deepUnknownObject(depth: number, leafSize: number, seed: number): Record<string, JsonValue> {
  const rng = createRng(seed);
  let node: JsonValue = makeToken(rng, "leaf-", leafSize);
  for (let index = 0; index < depth; index += 1) {
    node = { [`layer_${index.toString().padStart(2, "0")}`]: node };
  }
  return node as Record<string, JsonValue>;
}

function makeMalformedCase(index: number): BenchmarkCase {
  ...
  const baseValue = JSON.parse(rawJson(inflated as unknown as JsonValue)) as Record<string, JsonValue>;
  const mutationType = index % 6;
  let rawInput = "";
  let depth = computeDepth(baseValue as unknown as JsonValue);
  const keyOrderSeed = seed ^ 0x55aa55aa;

  if (mutationType === 0) {
    const duplicateRequest = `{"execution_request":{"request_id":"${caseId}-a","request_id":"${caseId}-b"},"policy_document":${JSON.stringify(
      inflated.policy_document
    )},"pricing_data":${JSON.stringify(inflated.pricing_data)}}`;
    rawInput = duplicateRequest;
    depth = 3;
  } else if (mutationType === 1) {
    (baseValue.execution_request as Record<string, JsonValue>).unknown_field = deepUnknownObject(4 + (index % 8), 256, seed);
    rawInput = serializeJson(baseValue as JsonValue, createRng(keyOrderSeed), 0);
    depth = computeDepth(baseValue as JsonValue);
  } else if (mutationType === 2) {
    delete (baseValue.pricing_data as Record<string, JsonValue>).gpu_hour_cents;
    rawInput = serializeJson(baseValue as JsonValue, createRng(keyOrderSeed), 2);
  } else if (mutationType === 3) {
    const asText = serializeJson(baseValue as JsonValue, createRng(keyOrderSeed), 0);
  ...
  return {
    ...
    tags: ["adversarial_malformed", "invalid"]
  };
}

export function buildBenchmarkMatrix(): BenchmarkCase[] {
...
export function getParityVectors(cases: BenchmarkCase[]): Array<{ case_id: string; raw_input: string }> {
  const output: Array<{ case_id: string; raw_input: string }> = [];
  for (const profile of PROFILE_ORDER) {
    if (profile === "adversarial_malformed") {
      continue;
    }
    ...
    for (const testCase of profileCases) {
      output.push({ case_id: testCase.case_id, raw_input: testCase.raw_input });
    }
  }
  return output;
}

export function getCanonicalizationBaseCases(cases: BenchmarkCase[]): BenchmarkCase[] {
...
export function writeParityVectors(cases: BenchmarkCase[]): void {
  ensureOutputDirs();
  writeRustParityVectors(PARITY_VECTOR_PATH, getParityVectors(cases));
}

export function buildMatrixReport(cases: BenchmarkCase[]) {
  return {
...
export function buildCanonicalVariant(baseCase: BenchmarkCase, variantIndex: number): string {
  const base = JSON.parse(baseCase.raw_input) as Record<string, JsonValue>;
  const rng = createRng(900000 + variantIndex + baseCase.key_order_seed);
  const category = variantIndex % 4;
  const variant = JSON.parse(JSON.stringify(base)) as Record<string, JsonValue>;
  const executionRequest = variant.execution_request as Record<string, JsonValue>;

  if (category === 0) {
    return serializeJson(variant as JsonValue, rng, 0);
  }

  if (category === 1) {
    const compact = serializeJson(variant as JsonValue, rng, 2);
    return compact.replace(/user/g, "\\u0075ser");
  }

  if (category === 2) {
    (executionRequest.execution as Record<string, JsonValue>).max_retries = variantIndex % 8 === 0 ? -0 : 0;
    return serializeJson(variant as JsonValue, rng, 2);
  }
...
export function canonicalBytes(value: JsonValue): string {
