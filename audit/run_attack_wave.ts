...
function withUnknownField(field: string, value: JsonValue) {
  const base = baseInput() as unknown as Record<string, unknown>;
...
export function buildAttackWaveCases(): AttackWaveCase[] {
  const cases: AttackWaveCase[] = [];
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          resources: { gpu_type: "a10g", gpu_count: 2147483647, hours: 2147483647 },
          execution: { auto_scale: true, max_scale_multiplier: 2147483647, retry_on_fail: true, max_retries: 2147483647 },
          runtime_observation: { kill_switch_active: false, actual_gpu_count: 0, actual_hours: 0, actual_total_cost_cents: 0 }
        }
      }) as unknown as JsonValue
    ),
    expected_outcome: "REFUSE",
  ...
  cases.push({
    ...
    raw_input: rawJson(
      baseInput({
        policy_document: {
          schema_version: "ecs.policy.v0" as never,
  ...
  cases.push({
    ...
    mode: "pipeline",
    raw_input:
      '{"execution_request":{"request_id":"num-049","submitted_region":"us-west-2","actor":{"user_id":"user-001"},"resources":{"gpu_type":"a10g","gpu_count":2,"hours":4},"execution":{"auto_scale":false,"max_scale_multiplier":1,"retry_on_fail":false,"max_retries":3},"tool_calls":[{"tool":"compile","priority":1},{"tool":"verify","priority":2}],"orbit_intent":{"orbit_version":"2.0","action":"execute","boundary":"gpu-batch","payload":{"tool_calls":[{"tool":"compile","priority":1},{"tool":"verify","priority":2}]},"lifecycle_state":"ARMED","signatures":[{"alg":"hmac-sha256","sig":"orbit-signature-v1"}]},"release_request":{"execution_id":"exec-049","hold_state":"APPROVED","already_consumed":false},"runtime_observation":{"kill_switch_active":false,"actual_gpu_count":2,"actual_hours":4,"actual_total_cost_cents":4000}},"policy_document":{"schema_version":"ecs.policy.v1","policy_version":"policy.v1","rules":{"max_total_cost_cents":500000,"allow_auto_scale":true,"max_gpu_count":32,"max_hours":72,"require_manual_approval_above_cents":250000,"max_retry_count":5}},"pricing_data":{"gpu_hour_cents":-0}}',
    expected_outcome: "REFUSE",
    expected_reason_code: "ERR_INVALID_JSON_NUMBER"
  });
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          release_request: { execution_id: "exec-051", hold_state: "APPROVED", already_consumed: false },
          parameters: {
            code: "rm -rf /data"
          }
        }
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          release_request: { execution_id: "exec-052", hold_state: "APPROVED", already_consumed: false },
          parameters: {
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          release_request: { execution_id: "exec-053", hold_state: "APPROVED", already_consumed: false },
          parameters: {
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          orbit_intent: {
            ...baseInput().execution_request.orbit_intent,
            payload: { tool_calls: [{ tool: "delete_database", priority: 1 }] }
          }
        }
      }) as unknown as JsonValue
    ),
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          release_request: { execution_id: "exec-055", hold_state: "APPROVED", already_consumed: false },
          parameters: {
            plan: [{ step: "retry_until_success", limit: "none" }]
          }
        }
      }) as unknown as JsonValue
    ),
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          release_request: { execution_id: "exec-race-2", hold_state: "APPROVED", already_consumed: false }
        }
      }) as unknown as JsonValue
    ),
    expected_outcome: "REFUSE",
    ...
    raw_input: rawJson(
      baseInput({
        execution_request: {
          ...
          release_request: { execution_id: "exec-replay-1", hold_state: "APPROVED", already_consumed: false }
        }
      }) as unknown as JsonValue
    ),
    expected_outcome: "REFUSE",
  ...
  cases.push({
    ...
    mode: "receipt_verify",
    build_receipt: () => {
      const first = executeDeterministicPipeline(rawJson(baseInput() as unknown as JsonValue));
      if ("parse_boundary" in first) {
        throw new Error("baseline receipt generation failed");
      }
      const receipt = JSON.parse(first.receipt_bytes) as SignedReceipt;
      receipt.canonical_request = rawJson(
        baseInput({
          execution_request: {
...
function executeCase(testCase: AttackWaveCase): AttackWaveResult {
  resetRuntimeState();
  testCase.setup?.();
