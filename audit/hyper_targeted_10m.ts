...
type BenchmarkWrapperReceipt = {
  ...
  pipeline_trace: {
    preflight: { layer: "Preflight"; canonicalization: "RAW_INPUT"; request_hash: string };
    orbit: { layer: "Orbit"; decision: "FAIL"; reason_code: string; validation_hash: string };
    arm: {
      layer: "ARM";
      decision: "REFUSE";
      reason_code: string;
    ...
    ram0na: { layer: "RAM0NA"; decision: "REFUSE"; reason_code: string; runtime_hash: string };
  };
  signature: { algorithm: "HMAC-SHA256"; key_id: string; value: string };
};

type SignedExecution = {
  receipt_bytes: string;
  receipt: SignedReceipt | BenchmarkWrapperReceipt;
  decision: "ALLOW" | "REFUSE";
  decision_hash: string;
  reason_code: string;
  canonical_request: string;
...
type ScenarioResult = {
  ...
  simulated_before_cost_cents: number;
  raw_input: string;
  parity_eligible: boolean;
};

type Histogram = {
  bucket_width_us: number;
  max_bucket: number;
  counts: Uint32Array;
  overflow: number;
  total: number;
...
function createHistogram(bucketWidthUs = 10, maxMs = 10): Histogram {
  const maxBucket = Math.ceil((maxMs * 1000) / bucketWidthUs);
  return { bucket_width_us: bucketWidthUs, max_bucket: maxBucket, counts: new Uint32Array(maxBucket + 1), overflow: 0, total: 0 };
}

function observeHistogram(histogram: Histogram, latencyMs: number): void {
...
function percentileFromHistogram(histogram: Histogram, ratio: number): number {
...
function resetHistogram(histogram: Histogram): Histogram {
...
function sleepMs(value: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, value));
}

function buildWeightedSchedule(): PrimaryCategory[] {
  return [
    ...Array.from({ length: 20 }, () => "valid_high_cost_allow" as const),
    ...Array.from({ length: 10 }, () => "valid_boundary_allow" as const),
    ...Array.from({ length: 15 }, () => "violation_autoscale_spike" as const),
    ...Array.from({ length: 15 }, () => "violation_retry_storm" as const),
    ...Array.from({ length: 10 }, () => "violation_long_running_cost" as const),
    ...Array.from({ length: 5 }, () => "violation_runtime_gpu_drift" as const),
    ...Array.from({ length: 5 }, () => "violation_tool_sequence" as const),
    ...Array.from({ length: 8 }, () => "malformed_json" as const),
    ...Array.from({ length: 5 }, () => "duplicate_keys" as const),
    ...Array.from({ length: 4 }, () => "unknown_fields" as const),
    ...Array.from({ length: 3 }, () => "missing_required_fields" as const)
  ];
}

function projectedCostCentsFromParsed(input: ReturnType<typeof makeBaseInput>): number {
...
function buildScenarioRequest(category: PrimaryCategory, sequence: number): ScenarioResult {
  if (category === "valid_high_cost_allow") {
    ...
    const input = makeBaseInput({
      execution_request: {
        ...
        release_request: { execution_id: `exec-valid-high-${sequence}`, hold_state: "APPROVED", already_consumed: false },
        runtime_observation: {
    ...
    return { category, expected_reason: "OK_ALLOW", simulated_before_cost_cents: projectedCostCentsFromParsed(input), raw_input: rawJson(input as unknown as JsonValue), parity_eligible: true };
  }
  if (category === "valid_boundary_allow") {
    const input = makeBaseInput({
      execution_request: {
        request_id: `valid-boundary-${sequence}`,
        resources: { gpu_type: "a10g", gpu_count: 32, hours: 10 },
        execution: { auto_scale: false, max_scale_multiplier: 1, retry_on_fail: false, max_retries: 0 },
        release_request: { execution_id: `exec-valid-boundary-${sequence}`, hold_state: "APPROVED", already_consumed: false },
        runtime_observation: { kill_switch_active: false, actual_gpu_count: 32, actual_hours: 10, actual_total_cost_cents: 160000 }
      }
    });
    return { category, expected_reason: "OK_ALLOW", simulated_before_cost_cents: projectedCostCentsFromParsed(input), raw_input: rawJson(input as unknown as JsonValue), parity_eligible: true };
  }
  if (category === "violation_autoscale_spike") {
    const input = makeBaseInput({
      execution_request: {
        ...
        resources: { gpu_type: "a10g", gpu_count: 24, hours: 48 },
        execution: { auto_scale: true, max_scale_multiplier: 28 + (sequence % 5), retry_on_fail: false, max_retries: 0 },
        release_request: { execution_id: `exec-viol-autoscale-${sequence}`, hold_state: "PENDING", already_consumed: false },
        runtime_observation: { kill_switch_active: false, actual_gpu_count: 24, actual_hours: 48, actual_total_cost_cents: 576000 }
      }
    });
    return { category, expected_reason: "ERR_MANUAL_APPROVAL_REQUIRED", simulated_before_cost_cents: projectedCostCentsFromParsed(input), raw_input: rawJson(input as unknown as JsonValue), parity_eligible: true };
  }
  if (category === "violation_retry_storm") {
    const input = makeBaseInput({
      execution_request: {
        request_id: `viol-retry-${sequence}`,
        resources: { gpu_type: "a10g", gpu_count: 16, hours: 24 },
        execution: { auto_scale: false, max_scale_multiplier: 1, retry_on_fail: true, max_retries: 60 + (sequence % 41) },
        release_request: { execution_id: `exec-viol-retry-${sequence}`, hold_state: "APPROVED", already_consumed: false },
        runtime_observation: { kill_switch_active: false, actual_gpu_count: 16, actual_hours: 24, actual_total_cost_cents: 192000 }
      }
    });
    return { category, expected_reason: "ERR_RETRY_LIMIT", simulated_before_cost_cents: projectedCostCentsFromParsed(input), raw_input: rawJson(input as unknown as JsonValue), parity_eligible: true };
  }
  if (category === "violation_long_running_cost") {
    const input = makeBaseInput({
      execution_request: {
        request_id: `viol-long-${sequence}`,
        resources: { gpu_type: "a10g", gpu_count: 32, hours: 72 },
        execution: { auto_scale: false, max_scale_multiplier: 1, retry_on_fail: false, max_retries: 0 },
        release_request: { execution_id: `exec-viol-long-${sequence}`, hold_state: "APPROVED", already_consumed: false },
        runtime_observation: { kill_switch_active: false, actual_gpu_count: 32, actual_hours: 72, actual_total_cost_cents: 1152000 }
      }
    });
    return { category, expected_reason: "ERR_COST_LIMIT", simulated_before_cost_cents: projectedCostCentsFromParsed(input), raw_input: rawJson(input as unknown as JsonValue), parity_eligible: true };
  }
  if (category === "violation_runtime_gpu_drift") {
    const input = makeBaseInput({
      execution_request: {
        request_id: `viol-runtime-${sequence}`,
        resources: { gpu_type: "a10g", gpu_count: 8, hours: 12 },
        execution: { auto_scale: false, max_scale_multiplier: 1, retry_on_fail: false, max_retries: 0 },
        release_request: { execution_id: `exec-viol-runtime-${sequence}`, hold_state: "APPROVED", already_consumed: false },
        runtime_observation: { kill_switch_active: false, actual_gpu_count: 10, actual_hours: 12, actual_total_cost_cents: 48000 }
      }
    });
    return { category, expected_reason: "ERR_RUNTIME_GPU_DRIFT", simulated_before_cost_cents: projectedCostCentsFromParsed(input), raw_input: rawJson(input as unknown as JsonValue), parity_eligible: true };
  }
  if (category === "violation_tool_sequence") {
    const input = makeBaseInput({
      execution_request: {
        request_id: `viol-tool-${sequence}`,
        tool_calls: [{ tool: "verify", priority: 2 }, { tool: "compile", priority: 1 }],
        orbit_intent: { payload: { tool_calls: [{ tool: "compile", priority: 1 }, { tool: "verify", priority: 2 }] } }
      }
    });
    return { category, expected_reason: "ERR_TOOL_CALL_SEQUENCE", simulated_before_cost_cents: projectedCostCentsFromParsed(input), raw_input: rawJson(input as unknown as JsonValue), parity_eligible: true };
  }
  if (category === "malformed_json") {
    return { category, expected_reason: "ERR_INVALID_JSON_SYNTAX", simulated_before_cost_cents: 0, raw_input: "{\"execution_request\":", parity_eligible: false };
  }
  ...
  if (category === "unknown_fields") {
    const base = makeBaseInput({ execution_request: { request_id: `unknown-${sequence}` } });
    return {
      category,
      expected_reason: "ERR_SCHEMA_VALIDATION",
      simulated_before_cost_cents: 0,
      raw_input: JSON.stringify({ ...base, execution_request: { ...base.execution_request, unknown_control: true } }),
      parity_eligible: false
    };
  }
  const base = makeBaseInput({ execution_request: { request_id: `missing-${sequence}` } });
  const value = JSON.parse(rawJson(base as unknown as JsonValue)) as Record<string, JsonValue>;
  delete (value.pricing_data as Record<string, JsonValue>).gpu_hour_cents;
  return { category, expected_reason: "ERR_SCHEMA_VALIDATION", simulated_before_cost_cents: 0, raw_input: JSON.stringify(value), parity_eligible: false };
}

function makeWrappedReceipt(rawInput: string, reasonCode: string): SignedExecution {
  ...
  const payload = {
    schema_version: "ecs.benchmark.receipt.v1" as const,
    request_hash: requestHash,
    ...
    pipeline_trace: {
      preflight: { layer: "Preflight" as const, canonicalization: "RAW_INPUT" as const, request_hash: requestHash },
      orbit: { layer: "Orbit" as const, decision: "FAIL" as const, reason_code: reasonCode, validation_hash: sha256Hex(rawInput) },
      arm: { layer: "ARM" as const, decision: "REFUSE" as const, reason_code: reasonCode, projected_total_cost_cents: 0, allowed_cost_cents: 0, prevented_cost_cents: 0 },
      ram0na: { layer: "RAM0NA" as const, decision: "REFUSE" as const, reason_code: reasonCode, runtime_hash: sha256Hex(reasonCode) }
    }
  };
  const signature = { algorithm: "HMAC-SHA256" as const, key_id: HASH_RECEIPT_KEY_ID, value: hmacHex(canonicalizeJson(payload as unknown as JsonValue)) };
  const receipt: BenchmarkWrapperReceipt = { ...payload, signature };
  return { receipt_bytes: canonicalizeJson(receipt as unknown as JsonValue), receipt, decision: "REFUSE", decision_hash: decisionHash, reason_code: reasonCode, canonical_request: rawInput, request_hash: requestHash, total_cost_cents: 0, allowed_cost_cents: 0, prevented_cost_cents: 0 };
}
...
function executeSigned(rawInput: string): SignedExecution {
...
function receiptSha256(receiptBytes: string): string {
  return sha256Hex(receiptBytes);
}

function stabilityBand(values: number[]): { min: number; max: number; mean: number; within_10_percent_band: boolean } {
  const min = Math.min(...values);
  const max = Math.max(...values);
  const mean = values.reduce((sum, value) => sum + value, 0) / values.length;
  const maxDeviation = Math.max(Math.abs(max - mean), Math.abs(min - mean));
  return { min: Number(min.toFixed(6)), max: Number(max.toFixed(6)), mean: Number(mean.toFixed(6)), within_10_percent_band: mean === 0 ? true : (maxDeviation / mean) <= 0.1 };
}

async function runPhase(): Promise<void> {
  mkdirSync(OUTPUT_DIR, { recursive: true });
  const fullReceiptStream = createWriteStream(SIGNED_RECEIPTS_PATH, { flags: "w" });
  const hashStream = createWriteStream(HASH_LOG_PATH, { flags: "w" });
  const schedule = buildWeightedSchedule();
  const throughputPerMinute: number[] = [];
  const latencyTimeseries: Array<Record<string, unknown>> = [];
  const resourceUsage: Array<Record<string, unknown>> = [];
  const refusalDistribution: Record<string, number> = {};
  const categoryDistribution: Record<string, number> = {};
  const preventedCostByCategory: Record<string, number> = {};
  const driftFailures: Array<Record<string, string | number>> = [];
  const adversarialFailures: Array<Record<string, string>> = [];
  ...
  const throughputMinuteCounts = Array.from({ length: RUN_MINUTES }, () => 0);
  ...
  let fiveMinuteDecisionDistribution = { ALLOW: 0, REFUSE: 0 };
  ...
  for (let secondIndex = 0; secondIndex < RUN_SECONDS; secondIndex += 1) {
    ...
    for (let perSecondIndex = 0; perSecondIndex < TARGET_RPS; perSecondIndex += 1) {
      ...
      if (outcome.reason_code === scenario.expected_reason) {
        ...
        adversarialFailures.push({ category, expected_reason: scenario.expected_reason, actual_reason: outcome.reason_code });
      }
      ...
      if (primaryExecutions % SAMPLE_FULL_RECEIPTS_EVERY === 0) {
        ...
        hashStream.write(`${JSON.stringify({ request_hash: outcome.request_hash, decision_hash: outcome.decision_hash, receipt_sha256: receiptSha256(outcome.receipt_bytes), category })}\n`);
        hashOnlyReceipts += 1;
      }

      if (scenario.parity_eligible && parityMirrors < PARITY_SAMPLE_TARGET && primaryExecutions % 600 === 0) {
        parityVectors.push({ case_id: `parity-${primaryExecutions}`, raw_input: scenario.raw_input });
        parityMirrors += 1;
      }
    }
    ...
    for (let replayIndex = 0; replayIndex < driftChecksThisSecond; replayIndex += 1) {
      ...
      if (next.receipt_bytes !== deterministicBaseline.receipt_bytes && driftFailures.length < 25) {
        driftFailures.push({ replay_index: driftChecks, expected_receipt: deterministicBaseline.receipt_bytes, actual_receipt: next.receipt_bytes });
      }
    }
    ...
    if ((secondIndex + 1) % FIVE_MINUTE_INTERVAL_SECONDS === 0) {
      ...
      fiveMinuteDecisionDistribution = { ALLOW: 0, REFUSE: 0 };
  ...
  await new Promise<void>((resolve) => fullReceiptStream.end(resolve));
  await new Promise<void>((resolve) => hashStream.end(resolve));

  if (parityVectors.length < PARITY_SAMPLE_TARGET) {
    const sampleReader = readline.createInterface({ input: createReadStream(SIGNED_RECEIPTS_PATH, { encoding: "utf8" }), crlfDelay: Infinity });
    for await (const line of sampleReader) {
      if (line.length === 0 || parityVectors.length >= PARITY_SAMPLE_TARGET) {
        continue;
      }
      const receipt = JSON.parse(line) as SignedReceipt | BenchmarkWrapperReceipt;
      if (receipt.schema_version !== "ecs.receipt.v1") {
        continue;
      }
      parityVectors.push({ case_id: `parity-backfill-${parityVectors.length.toString().padStart(4, "0")}`, raw_input: receipt.canonical_request });
    }
  }
  ...
  const memoryTrend = resourceUsage.length < 2
    ? { no_upward_trend: true, first: resourceUsage[0]?.memory_rss_bytes ?? 0, last: resourceUsage[0]?.memory_rss_bytes ?? 0 }
    : { no_upward_trend: Number(resourceUsage.at(-1)?.memory_rss_bytes ?? 0) <= Number(resourceUsage[0]?.memory_rss_bytes ?? 0) * 1.1, first: Number(resourceUsage[0]?.memory_rss_bytes ?? 0), last: Number(resourceUsage.at(-1)?.memory_rss_bytes ?? 0) };
  ...
  writeJsonArtifact(COST_ANALYSIS_PATH, {
    ...
    top_prevented_scenarios: Object.entries(preventedCostByCategory).sort((left, right) => right[1] - left[1]).slice(0, 3).map(([category, cents]) => ({ category, prevented_cost_cents: cents }))
  } as unknown as JsonValue);
  ...
  writeJsonArtifact(LATENCY_TIMESERIES_PATH, latencyTimeseries as unknown as JsonValue);
  writeJsonArtifact(THROUGHPUT_TIMESERIES_PATH, throughputPerMinute as unknown as JsonValue);
  writeJsonArtifact(RESOURCE_USAGE_PATH, resourceUsage as unknown as JsonValue);
...
async function replayStoredReceipts(): Promise<{ total_replays: number; mismatch_count: number; drift_rate: number; failures: Array<Record<string, string>> }> {
  const failures: Array<Record<string, string>> = [];
  let mismatches = 0;
  let total = 0;
  const reader = readline.createInterface({ input: createReadStream(SIGNED_RECEIPTS_PATH, { encoding: "utf8" }), crlfDelay: Infinity });
  for await (const line of reader) {
    ...
    if (next.receipt_bytes !== line) {
      ...
      if (failures.length < 25) {
        failures.push({ request_hash: receipt.request_hash, expected_receipt: line, actual_receipt: next.receipt_bytes });
      }
    }
  }
  return { total_replays: total, mismatch_count: mismatches, drift_rate: percent(mismatches, total), failures };
}
...
async function finalizePhase(): Promise<void> {
  const state = JSON.parse(readFileSync(RUN_STATE_PATH, "utf8")) as Record<string, any>;
  const parityVectors = JSON.parse(readFileSync(PARITY_VECTORS_PATH, "utf8")) as Array<{ case_id: string; raw_input: string }>;
  const rustOutputs = JSON.parse(readFileSync(RUST_PARITY_OUTPUT_PATH, "utf8")) as Array<{ case_id: string; receipt_bytes: string; decision_hash: string; decision: string }>;
  ...
  const parityResults = parityVectors.map((vector, index) => {
    ...
    return { case_id: vector.case_id, receipt_bytes_identical: node.receipt_bytes === rust.receipt_bytes, decision_hash_identical: node.decision_hash === rust.decision_hash, decision_identical: node.decision === rust.decision };
  });
  ...
  const parityReport = { schema_version: "ecs.audit.hyper_targeted.parity_report.v1", total_comparisons: parityResults.length, mismatch_count: parityMismatchCount, results: parityResults };
  ...
  const summary = {
    ...
    top_3_prevented_high_cost_scenarios: Object.entries(state.prevented_cost_by_category).sort((left, right) => Number(right[1]) - Number(left[1])).slice(0, 3).map(([category, cents]) => ({ category, prevented_cost_cents: cents })),
    latency_distribution: { p50_ms: state.latency_timeseries.at(-1)?.p50_ms ?? 0, p95_ms: state.latency_timeseries.at(-1)?.p95_ms ?? 0, p99_ms: state.latency_timeseries.at(-1)?.p99_ms ?? 0 },
    determinism: JSON.parse(readFileSync(DRIFT_REPORT_PATH, "utf8")),
    replay: replayReport,
    parity: { total_comparisons: parityReport.total_comparisons, mismatch_count: parityReport.mismatch_count }
  };
  ...
  const finalSummary = {
    ...
    parity_results: { total_comparisons: parityReport.total_comparisons, mismatch_count: parityReport.mismatch_count },
    latency_summary: { p50_ms: state.latency_timeseries.at(-1)?.p50_ms ?? 0, p95_ms: state.latency_timeseries.at(-1)?.p95_ms ?? 0, p99_ms: state.latency_timeseries.at(-1)?.p99_ms ?? 0 },
    pass_fail: {
  ...
  writeJsonArtifact(REPLAY_REPORT_PATH, replayReport as unknown as JsonValue);
  writeJsonArtifact(PARITY_REPORT_PATH, parityReport as unknown as JsonValue);
  writeJsonArtifact(SUMMARY_PATH, summary as unknown as JsonValue);
  writeJsonArtifact(FINAL_SUMMARY_PATH, finalSummary as unknown as JsonValue);
  ...
  writeJsonArtifact(MANIFEST_PATH, {
    ...
    artifacts: await Promise.all(manifestFiles.map(async (file) => ({ file, sha256: await hashFile(join(OUTPUT_DIR, file)) })))
  } as unknown as JsonValue);
}
