...
import type { JsonValue, SignedReceipt } from "../shared/index.ts";
...
function runCase(caseDef: any): CaseResult {
  resetRuntimeState();
...
function runSequentialDouble(rawInput: string): { first: CaseResult; second: CaseResult } {
  resetRuntimeState();
  const firstRaw = executeDeterministicPipeline(rawInput);
  const first =
    "parse_boundary" in firstRaw
      ? {
...
function resultFingerprint(result: unknown): string {
  return canonicalizeJson(result as JsonValue);
}
...
function runDriftChecks() {
  ...
  for (const caseDef of cases) {
    ...
    if (caseDef.mode === "pipeline") {
      ...
      sampleResults.push({ test_id: caseDef.test_id, first, second });
    }
  }
...
function runBudgetStress() {
  ...
  for (let index = 0; index < 50; index += 1) {
    ...
    const firstInput = rawJson(
      makeBaseInput({
        execution_request: {
          request_id: `budget-allow-${index}`,
          release_request: { execution_id: `budget-exec-a-${index}`, hold_state: "APPROVED", already_consumed: false },
          budget_token: token
        }
      }) as unknown as JsonValue
    );
    const secondInput = rawJson(
      makeBaseInput({
        execution_request: {
          request_id: `budget-refuse-${index}`,
          release_request: { execution_id: `budget-exec-b-${index}`, hold_state: "APPROVED", already_consumed: false },
          budget_token: token
        }
      }) as unknown as JsonValue
    );
    ...
    cases.push({ token, firstDecision, firstOutcome, secondDecision, secondOutcome });
  }
  return {
...
function main() {
  ...
  const unexpectedAllowCount =
    attackWave.cases.filter((item: any) => item.expected_outcome === "REFUSE" && item.actual_outcome === "ALLOW").length +
    remediationWave.cases.filter((item: any) => item.expected_outcome === "REFUSE" && item.actual_outcome === "ALLOW").length;
  const doubleAllowCount = concurrency.duplicate_allows;
  const genericSchemaFallbackCount =
    attackWave.cases.filter((item: any) => item.actual_reason_code === "ERR_SCHEMA_VALIDATION").length +
    remediationWave.cases.filter((item: any) => item.actual_reason_code === "ERR_SCHEMA_VALIDATION").length;

  const regressionSummary = {
    semantic_intent_blocking: remediationWave.cases.filter((item: any) => ["TC-061", "TC-062", "TC-063", "TC-064"].includes(item.test_id)).every((item: any) => item.status === "PASS"),
    execution_id_single_use: remediationWave.cases.filter((item: any) => ["TC-065", "TC-066"].includes(item.test_id)).every((item: any) => item.status === "PASS"),
    budget_token_enforcement: remediationWave.cases.find((item: any) => item.test_id === "TC-067")?.status === "PASS",
    policy_version_pinning: remediationWave.cases.find((item: any) => item.test_id === "TC-068")?.status === "PASS",
    policy_trust_validation: remediationWave.cases.filter((item: any) => ["TC-069", "TC-070"].includes(item.test_id)).every((item: any) => item.status === "PASS"),
    invalid_number_precision: remediationWave.cases.find((item: any) => item.test_id === "TC-072")?.status === "PASS",
    duplicate_key_precedence: remediationWave.cases.find((item: any) => item.test_id === "TC-071")?.status === "PASS"
  };

  const totalTestsRun =
    5 +
    attackWave.summary.total_cases +
    remediationWave.summary.total_cases +
    drift.repeated_inputs +
    replay.total +
    100 +
    100 +
    4;
  ...
  const totalFailed =
    (legacyDeterminism.mismatch_count > 0 ? 1 : 0) +
    (legacyReplay.drift_count > 0 ? 1 : 0) +
    (legacyParity.mismatch_count > 0 ? 1 : 0) +
    (legacySummary.rejection_accuracy !== 100 ? 1 : 0) +
    (legacySummary.determinism_mismatch_rate !== 0 ? 1 : 0) +
    attackWave.summary.fail_count +
    remediationWave.summary.fail_count +
    drift.drift_mismatch_count +
    replay.mismatches.length +
    concurrency.duplicate_allows +
  ...
  const finalVerdict =
    drift.drift_mismatch_count > 0 || replay.mismatches.length > 0
      ? "FAIL_DETERMINISM_GAP"
      : unexpectedAllowCount > 0 || doubleAllowCount > 0
        ? "FAIL_AUTHORITY_GAP"
        : legacyParity.mismatch_count > 0 || attackWave.summary.fail_count > 0
          ? "FAIL_REGRESSION_DETECTED"
          : "PASS_READY_FOR_PROOF_EXPANSION";
  ...
  writeJsonArtifact(REPORT_PATH, report as unknown as JsonValue);
