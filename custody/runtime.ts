...
import { canonicalPolicyPayload, deriveKeyId, policyHash, verifyPolicySignature } from "../shared/policy-trust.ts";
import {
  appendReceiptRecord,
  buildHealthState,
  checkDiskStatus,
  OPS_ERRORS,
  rotateReceiptsIfNeeded,
  writeBoundedLog
} from "../shared/operations.js";
import { type ReleaseIntegrityResult, verifyReleaseIntegrity } from "../release/integrity.ts";
import { PACKAGE_ROOT } from "../release/paths.ts";
...
type ReceiptStoreConfig = {
  path: string;
  archive_path: string;
  rotation_mode: "size" | "count";
  max_bytes: number;
  max_count: number;
  simulate_write_failure?: boolean;
  simulate_read_failure?: boolean;
};

export type CustodyStartupResult = {
  ok: boolean;
  code: string;
  reason?: string;
  ...
  preflight_lock?: Record<string, unknown> | null;
};
...
type CustodyConfig = {
  ...
  runtime: {
    bind: string;
    deny_internal_signing: boolean;
    fail_on_forbidden_artifacts: boolean;
    health_path: string;
    ready_path: string;
  };
  logging: {
    ...
    max_bytes: number;
    max_files: number;
    required_for_audit_integrity: boolean;
  };
  receipts: ReceiptStoreConfig;
  disk: {
    min_free_bytes: number;
    simulated_free_bytes?: number;
  };
  signer: {
    timeout_ms: number;
...
type PolicyValidationResult =
  | {
      ok: true;
      code: "OK_POLICY_READY" | "OK_POLICY_DISABLED";
  ...
  | {
      ok: false;
      code:
        | typeof ERR_POLICY_PATH_MISSING
        | typeof ERR_POLICY_FILE_MISSING
        | typeof ERR_POLICY_FILE_UNREADABLE
        | typeof ERR_POLICY_JSON_PARSE_FAILED
        | typeof ERR_POLICY_SCHEMA_INVALID
        | typeof ERR_POLICY_SIGNATURE_INVALID
        | typeof ERR_POLICY_HASH_MISMATCH;
      policy: ActivePolicy | null;
      policy_hash: string | null;
      path: string | null;
      details: Record<string, unknown> | null;
...
function typedError(code: string, message: string, field?: string): Error & { code: string; field?: string } {
...
function requireString(value: Record<string, unknown>, key: string, expected?: string): string {
  const field = key;
...
function requireStringAllowEmpty(value: Record<string, unknown>, key: string): string {
  const field = key;
...
function requireBoolean(value: Record<string, unknown>, key: string, expected?: boolean): boolean {
  const field = key;
...
function requireOptionalString(value: Record<string, unknown>, key: string): string | null {
...
function requireOptionalBoolean(value: Record<string, unknown>, key: string): boolean | undefined {
  if (!(key in value)) {
    return undefined;
  }
  if (typeof value[key] !== "boolean") {
...
export function readCustodyConfig(configPath: string): CustodyConfig {
  let parsed: unknown;
  try {
    parsed = JSON.parse(readFileSync(configPath, "utf8"));
  } catch (error) {
    throw typedError(ERR_INVALID_CONFIG, `config file cannot be parsed: ${(error as Error).message}`, "config");
  }
  assertObject(parsed, "config");
  const config = parsed;
  const strict = requireBoolean(config, "strict", true);
  if (strict) {
    rejectUnknownKeys(config, ["schema_version", "mode", "strict", "service_name", "runtime", "logging", "receipts", "disk", "signer", "policy"], "config");
  }
  ...
  if (strict) {
    rejectUnknownKeys(config.runtime, ["bind", "deny_internal_signing", "fail_on_forbidden_artifacts", "health_path", "ready_path"], "runtime");
    rejectUnknownKeys(config.logging, ["path", "runtime_log", "install_log", "verification_log", "max_bytes", "max_files", "required_for_audit_integrity"], "logging");
    rejectUnknownKeys(config.receipts, ["path", "archive_path", "rotation_mode", "max_bytes", "max_count", "simulate_write_failure", "simulate_read_failure"], "receipts");
    rejectUnknownKeys(config.disk, ["min_free_bytes", "simulated_free_bytes"], "disk");
    rejectUnknownKeys(config.signer, ["timeout_ms", "simulate_timeout"], "signer");
    rejectUnknownKeys(config.policy, ["mode", "path", "expected_hash"], "policy");
  }
  ...
  if (!["size", "count"].includes(receiptMode)) {
  ...
  if (!["required", "disabled"].includes(policyMode)) {
  ...
  return config as CustodyConfig;
}
...
function hashConfig(config: CustodyConfig): string {
  return sha256Text(canonicalizeJson(config as unknown as import("../shared/json.ts").JsonValue));
}

function preflightLockPath(configPath: string): string {
  return `${configPath}.preflight-lock.json`;
}

function validationHashForPolicyInput(config: CustodyConfig, payload: string | null): string {
  return sha256Text(
    canonicalizeJson(
      {
        ...
        payload
      } as unknown as import("../shared/json.ts").JsonValue
    )
  );
}

function validateActivePolicyShape(policy: unknown): { ok: true; policy: ActivePolicy } | { ok: false; error: string } {
  if (!policy || typeof policy !== "object" || Array.isArray(policy)) {
    return { ok: false, error: "policy must be an object" };
  }
  const value = policy as Record<string, unknown>;
  if (value.schema_version !== "mnde.policy.v1") {
    return { ok: false, error: "schema_version must be mnde.policy.v1" };
  }
  ...
  const numericRuleKeys = ["max_total_cost_usd", "max_gpu_count", "max_hours", "require_manual_approval_above_usd"];
  for (const key of numericRuleKeys) {
    if (typeof rules[key] !== "number" || !Number.isFinite(rules[key])) {
      return { ok: false, error: `rules.${key} must be a finite number` };
    }
  }
  if (typeof rules.allow_auto_scale !== "boolean") {
    return { ok: false, error: "rules.allow_auto_scale must be boolean" };
  }
  if (!value.trust || typeof value.trust !== "object" || Array.isArray(value.trust)) {
    return { ok: false, error: "trust must be an object" };
  }
  const trust = value.trust as Record<string, unknown>;
  if (trust.key_version !== "ed25519.v1") {
    return { ok: false, error: "trust.key_version must be ed25519.v1" };
  }
  if (typeof trust.key_id !== "string" || trust.key_id.length === 0) {
    return { ok: false, error: "trust.key_id must be a non-empty string" };
  }
  if (typeof trust.signing_public_key !== "string" || !/^[0-9a-f]{64}$/i.test(trust.signing_public_key)) {
    return { ok: false, error: "trust.signing_public_key must be 32-byte hex" };
  }
  if (typeof trust.signature !== "string" || !/^[0-9a-f]+$/i.test(trust.signature) || trust.signature.length === 0) {
    return { ok: false, error: "trust.signature must be non-empty hex" };
  }
  return { ok: true, policy: value as ActivePolicy };
}

export function validateConfiguredPolicy(config: CustodyConfig): PolicyValidationResult {
  ...
  if (derivedKeyId !== shaped.policy.trust.key_id) {
    return {
      ...
      policy_hash: null,
      path: policyPath,
      details: { error: "trust.key_id does not match signing_public_key", expected_key_id: derivedKeyId },
  ...
  if (config.policy.expected_hash && actualHash !== config.policy.expected_hash) {
    return {
      ...
      path: policyPath,
      details: { expected_hash: config.policy.expected_hash, actual_hash: actualHash },
      validation_hash: validationHashForPolicyInput(config, payload)
    };
  }
  ...
  return {
    ...
    policy_hash: actualHash,
    path: policyPath,
    details: { policy_version: shaped.policy.policy_version },
    validation_hash: validationHashForPolicyInput(config, payload)
  };
}

function writeRuntimeLog(config: CustodyConfig, event: Record<string, unknown>): void {
  process.stdout.write(`${JSON.stringify({ ts: new Date().toISOString(), ...event })}\n`);
  writeBoundedLog(config.logging, event);
}

export function internalSigningDisabledError(): Error & { code: string } {
  const error = new Error(ERR_INTERNAL_SIGNING_DISABLED) as Error & { code: string };
  error.code = ERR_INTERNAL_SIGNING_DISABLED;
  return error;
}

export function signInternally(): never {
  throw internalSigningDisabledError();
}

export function assertCustodyOnlyBehavior(): { ok: true; code: string } {
  try {
    signInternally();
  } catch (error) {
    if ((error as Error & { code?: string }).code === ERR_INTERNAL_SIGNING_DISABLED) {
      return { ok: true, code: ERR_INTERNAL_SIGNING_DISABLED };
    }
    throw error;
  }
...
function safeReadJsonLine(filePath: string, lineNumberFromEnd = 1): { ok: true; data: Record<string, unknown> } | { ok: false; code: string; error: string } {
  try {
    const lines = readFileSync(filePath, "utf8").trim().split(/\r?\n/).filter(Boolean);
    const line = lines.at(-lineNumberFromEnd);
    if (!line) {
      return { ok: false, code: ERR_RECEIPT_READ_FAILED, error: "receipt_line_missing" };
    }
    return { ok: true, data: JSON.parse(line) as Record<string, unknown> };
  } catch (error) {
    return { ok: false, code: ERR_RECEIPT_READ_FAILED, error: (error as Error).message };
  }
}

function buildAuditReceiptPayload(result: CustodyStartupResult) {
  ...
  const policyIdValue =
    result.policy &&
    typeof result.policy === "object" &&
    result.policy.details &&
    typeof result.policy.details === "object" &&
    typeof (result.policy.details as Record<string, unknown>).policy_version === "string"
      ? ((result.policy.details as Record<string, unknown>).policy_version as string)
      : null;
  return {
...
function signAuditReceiptPayload(payload: Record<string, unknown>) {
  return {
    algorithm: "SHA256",
    value: sha256Text(canonicalizeJson(payload as unknown as import("../shared/json.ts").JsonValue))
  };
}

function buildAuditReceipt(result: CustodyStartupResult) {
  const payload = buildAuditReceiptPayload(result);
  const decision_hash = sha256Text(
    canonicalizeJson(
      {
  ...
  return {
    ...receipt,
    receipt_hash: sha256Text(canonicalizeJson(receipt as unknown as import("../shared/json.ts").JsonValue))
  };
}

function verifyAuditReceiptSignature(receipt: Record<string, unknown>): boolean {
  ...
  const value = (signature as Record<string, unknown>).value;
...
function writeProbeReceiptAndVerify(config: CustodyConfig, result: CustodyStartupResult): { ok: boolean; code: string; receipt?: Record<string, unknown>; error?: string } {
  const probeResult: CustodyStartupResult = {
    ...result,
    ok: true,
  ...
  try {
    if (config.receipts.simulate_write_failure === true) {
      return { ok: false, code: OPS_ERRORS.receiptWriteFailed, error: "simulated_write_failure" };
    }
    appendReceiptRecord(config.receipts, probeReceipt);
  } catch (error) {
    return {
      ok: false,
      code: (error as Error & { code?: string }).code ?? OPS_ERRORS.receiptWriteFailed,
      error: (error as Error).message
    };
  }
  if (config.receipts.simulate_read_failure === true) {
    return { ok: false, code: ERR_RECEIPT_READ_FAILED, error: "simulated_read_failure" };
  }
  const readBack = safeReadJsonLine(config.receipts.path);
  if (!readBack.ok) {
    return { ok: false, code: readBack.code, error: readBack.error };
  }
  if (!verifyAuditReceiptSignature(readBack.data)) {
    return { ok: false, code: ERR_RECEIPT_SIGNATURE_INVALID, error: "probe_signature_invalid" };
  }
  return { ok: true, code: "OK_RECEIPT_STORE_DURABLE", receipt: readBack.data };
}

function validatePreflightLock(configPath: string, configHash: string, policyHashValue: string | null) {
  const lockPath = preflightLockPath(configPath);
  if (!existsSync(lockPath)) {
    return { ok: false, code: ERR_PREFLIGHT_LOCK_MISSING, details: { lock_path: lockPath } };
  }
  try {
    const lock = JSON.parse(readFileSync(lockPath, "utf8")) as Record<string, unknown>;
    if (lock.config_hash !== configHash) {
      return { ok: false, code: ERR_PREFLIGHT_CONFIG_MISMATCH, details: { lock_path: lockPath, expected: lock.config_hash, actual: configHash }, lock };
    }
    if ((lock.policy_hash ?? null) !== policyHashValue) {
      return { ok: false, code: ERR_PREFLIGHT_POLICY_MISMATCH, details: { lock_path: lockPath, expected: lock.policy_hash ?? null, actual: policyHashValue }, lock };
    }
    return { ok: true, code: "OK_PREFLIGHT_LOCK", details: { lock_path: lockPath }, lock };
  } catch (error) {
    return { ok: false, code: ERR_PREFLIGHT_LOCK_MISSING, details: { lock_path: lockPath, error: (error as Error).message } };
  }
}

function policyForLockComparison(config: CustodyConfig): PolicyValidationResult {
...
export function checkCustodyStartup(
  packageRoot = PACKAGE_ROOT,
  config?: CustodyConfig,
  options: { configPath?: string } = {}
): CustodyStartupResult {
...
function writePathProbe(targetPath: string) {
...
export function runPreflightCheck(options: { configPath?: string; packageRoot?: string } = {}) {
  const checks: Array<Record<string, unknown>> = [];
  let config: CustodyConfig | null = null;

  const resolvedConfigPath = configPathFromEnvOrDefault(options.configPath);
  try {
    config = readCustodyConfig(resolvedConfigPath);
    checks.push({ name: "config", ok: true, code: "OK_CONFIG", path: resolvedConfigPath });
  } catch (error) {
    checks.push({
      name: "config",
      ok: false,
      code: ERR_INVALID_CONFIG,
      field: (error as Error & { field?: string }).field ?? null,
      error: (error as Error).message
    });
  }
  ...
  try {
    ...
    checks.push({ name: "bind", ok: true, code: "OK_BIND", value: config.runtime.bind });
  } catch (error) {
    checks.push({ name: "bind", ok: false, code: ERR_INVALID_CONFIG, error: (error as Error).message });
  }

  try {
    writePathProbe(config.receipts.path);
    checks.push({ name: "receipt_path", ok: true, code: "OK_RECEIPT_PATH_WRITABLE", path: config.receipts.path });
  } catch (error) {
    checks.push({ name: "receipt_path", ok: false, code: OPS_ERRORS.receiptWriteFailed, error: (error as Error).message });
  }

  try {
    writePathProbe(config.logging.path);
    checks.push({ name: "log_path", ok: true, code: "OK_LOG_PATH_WRITABLE", path: config.logging.path });
  } catch (error) {
    checks.push({ name: "log_path", ok: false, code: OPS_ERRORS.logPathUnavailable, error: (error as Error).message });
  }
  ...
  if (ok) {
    writeFileSync(
      lockPath,
      `${JSON.stringify({ schema_version: "mnde.custody.preflight_lock.v1", config_hash: configHash, policy_hash: policy.policy_hash ?? null }, null, 2)}\n`,
      "utf8"
    );
  }
...
export function logStartupResult(result: CustodyStartupResult, config?: CustodyConfig | null): void {
  if (result.ok) {
    process.stdout.write(`${JSON.stringify({ event: "mnde.custody.startup", decision: "ALLOW", reason_code: result.code })}\n`);
    return;
  }
    ...
    try {
      ...
      process.stderr.write(
        `${JSON.stringify({
          ...
          decision: "WARN",
          reason_code: (error as Error & { code?: string }).code ?? OPS_ERRORS.receiptWriteFailed,
          error: (error as Error).message
        })}\n`
      );
    }
...
function configPathFromEnvOrDefault(explicit?: string): string {
  return explicit ?? process.env.MNDE_CUSTODY_CONFIG ?? path.join(PACKAGE_ROOT, "config", "custody.config.template.json");
}

export function runCustodyRuntime(options: { once?: boolean; configPath?: string } = {}): void {
  ...
  const result = checkCustodyStartup(PACKAGE_ROOT, config, { configPath: resolvedConfigPath });
  ...
  let manifestOk = true;
  let logStatus = { ok: true, code: "OK_LOG_WRITTEN" };
  let receiptStatus = { ok: true, code: "OK_RECEIPT_STORE" };
  let signerStatus = { ok: true, code: "OK_SIGNER" };
  const custodyStatus = { ok: true, code: "OK_CUSTODY_READY" };

  function currentHealth() {
    const diskStatus = checkDiskStatus([config.logging.path, config.receipts.path, config.receipts.archive_path], config.disk);
    if (!diskStatus.ok) {
      receiptStatus = { ok: false, code: OPS_ERRORS.diskLow };
    }
    if (config.signer.simulate_timeout === true) {
      signerStatus = { ok: false, code: OPS_ERRORS.signatureTimeout };
    }
  ...
  try {
    ...
    receiptStatus = { ok: true, code: "OK_RECEIPT_STORE" };
  } catch (error) {
    receiptStatus = { ok: false, code: (error as Error & { code?: string }).code ?? OPS_ERRORS.receiptArchiveFailed };
  }

  const server = http.createServer((req, res) => {
    const pathname = new URL(req.url ?? "/", "http://127.0.0.1").pathname;
    if (req.method === "GET" && pathname === config.runtime.health_path) {
      const body = Buffer.from(JSON.stringify({ service_name: config.service_name, started_at: startupTime, ...currentHealth() }) + "\n");
    ...
    if (req.method === "GET" && pathname === config.runtime.ready_path) {
      ...
      res.writeHead(health.ready ? 200 : 503, { "content-type": "application/json", "content-length": body.byteLength });
      res.end(body);
      return;
    }
    res.writeHead(404, { "content-type": "application/json" });
    res.end(JSON.stringify({ ok: false, reason_code: "ERR_NOT_FOUND" }) + "\n");
  });
  server.on("error", (error) => {
    writeRuntimeLog(config, { event: "mnde.custody.startup", decision: "REFUSE", reason_code: "ERR_SERVICE_START_FAILED", error: error.message });
    process.exit(1);
  });
  server.listen(bind.port, bind.host, () => {
    logStatus = writeBoundedLog(config.logging, { event: "mnde.custody.startup", decision: "ALLOW", reason_code: "OK_READY", bind: config.runtime.bind });
  });
}

export function replayStartupReceipt(
  receipt: Record<string, unknown>,
  config: CustodyConfig,
  currentPolicyOverride?: { policy_hash: string | null; path: string | null }
) {
  if (!verifyAuditReceiptSignature(receipt)) {
    return { ok: false, code: ERR_RECEIPT_SIGNATURE_INVALID };
  }
  ...
  return { ok: true, code: "OK_RECEIPT_REPLAY" };
}
