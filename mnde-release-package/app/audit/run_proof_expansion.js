...
import { deriveKeyId } from "../shared/index.js";
import { ensureDir, executeDeterministicPipeline, makeBaseInput, rawJson, resetRuntimeState, seedBudgetToken, simulateConcurrentDuplicate, writeJsonArtifact } from "./node_runtime.js";
function baseInput(overrides) {
    return makeBaseInput(overrides);
}
function outcomeFromRaw(rawInput) {
...
function sequenceCase(test_id, category, intent, why_this_matters, steps, setup) {
...
function buildCases() {
    return [
        sequenceCase("PX-001", "multi-request interaction", "Semantic refusal must not consume budget for a later clean request sharing the same token", "Weak systems burn budget on refused pre-execution paths and then refuse clean follow-up work nondeterministically.", [
            {
                raw_input: rawJson(baseInput({
                    execution_request: {
                        request_id: "px-001-a",
                        release_request: {
                            execution_id: "px-001-a",
                            hold_state: "APPROVED",
        ...
        sequenceCase("PX-003", "multi-request interaction", "A previously allowed execution_id must refuse a later request even if the payload changes", "Weak systems key only on payload hash or only on execution_id and lose replay integrity.", [
            {
                raw_input: rawJson(baseInput({
                    execution_request: {
                        request_id: "px-003-a",
                        release_request: {
                            execution_id: "px-003-exec",
                            hold_state: "APPROVED",
                            already_consumed: false
                        }
                    }
        ...
        sequenceCase("PX-004", "multi-request interaction", "Shared budget token must allow within-budget use then refuse the overdraw attempt", "Double-spend against a shared budget object is a common cross-request authority failure.", [
            {
                raw_input: rawJson(baseInput({
                    execution_request: {
                        request_id: "px-004-a",
                        release_request: {
                            execution_id: "px-004-a",
                            hold_state: "APPROVED",
                            already_consumed: false
                        },
        ...
        sequenceCase("PX-012", "cross-policy transitions", "A valid request under the pinned policy must not make a later wrong-version request acceptable", "Weak systems leak policy state across requests or cache the first allow too broadly.", [
            {
                raw_input: rawJson(baseInput({
                    execution_request: {
                        request_id: "px-012-a",
                        release_request: {
                            execution_id: "px-012-a",
                            hold_state: "APPROVED",
                            already_consumed: false
                        }
                    }
            ...
            raw_input: rawJson(baseInput({
                execution_request: {
                    ...
                    runtime_observation: {
                        ...
                        actual_gpu_count: 20,
                        actual_hours: 10,
                        actual_total_cost_cents: 500000
                    }
                }
            })),
            expected_outcome: "ALLOW",
        ...
        },
        {
            ...
            raw_input: rawJson(baseInput({
                execution_request: {
                    ...
                    resources: {
                        gpu_type: "a10g",
                        gpu_count: 2_147_483_647,
                        hours: 2_147_483_647
                    },
                    execution: {
                        auto_scale: true,
                        max_scale_multiplier: 2_147_483_647,
                        retry_on_fail: true,
...
function main() {
    ...
    const results = cases.map((testCase)=>{
        ...
        if (testCase.mode === "concurrent") {
            ...
            return {
                ...
                status: concurrent.first.decision === "ALLOW" && concurrent.second.decision === testCase.expected_outcome && concurrent.second.reason_code === testCase.expected_reason_code ? "PASS" : "FAIL"
            };
        }
        const step_results = testCase.steps.map((step)=>{
            ...
            return {
                ...
                actual_outcome: outcome.decision,
                actual_reason_code: outcome.reason_code,
                status: outcome.decision === step.expected_outcome && outcome.reason_code === step.expected_reason_code ? "PASS" : "FAIL"
            };
        });
    ...
    const adversarial_scenarios = [
        {
            ...
            evidence: [
                "arm trace budget token",
                "receipt decision hash",
                "budget stress samples",
                "post verification budget summary"
            ]
        },
        {
            ...
            evidence: [
                "execution authority store snapshot",
                "concurrency summary",
                "receipt lineage by execution_id"
            ]
        },
        {
            ...
            evidence: [
                "orbit validation hash",
                "semantic attack case result",
    ...
    writeJsonArtifact(join(outputDir, "proof_expansion_results.json"), {
        summary,
    ...
    process.stdout.write(`${JSON.stringify({
        summary,
        output: join(outputDir, "proof_expansion_results.json")
    }, null, 2)}\n`);
}
main();
