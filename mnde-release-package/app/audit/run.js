...
import { appendReceipts, ensureDir, executeDeterministicPipeline, hashFileArtifact, makeBaseInput, rawJson, resetRuntimeState, replayReceiptStore, writeJsonArtifact } from "./node_runtime.js";
import { buildBenchmarkMatrix, buildCanonicalVariant, buildMatrixReport, canonicalBytes, ensureOutputDirs, FULL_LOGS_DIR, getCanonicalizationBaseCases, OUTPUT_DIR, PARITY_VECTOR_PATH, PROFILE_ORDER, PROOF_BUNDLE_DIR, RECEIPT_PATH, writeParityVectors } from "./harness.js";
...
function performancePercentDifference(sut, baseline) {
    if (baseline === 0) {
        return 0;
    }
    return Number(((sut - baseline) / baseline * 100).toFixed(6));
}
function measurePerformance(name, inputs, runner) {
    const latenciesMs = [];
...
function executeBaseline(rawInput) {
    try {
        ...
        if (release.already_consumed) {
            return {
                decision: "REFUSE",
                reason_code: "ERR_RELEASE_ALREADY_CONSUMED"
            };
        }
        ...
        if (projected > Number(rules.require_manual_approval_above_cents ?? 0) && release.hold_state !== "APPROVED") {
            return {
                decision: "REFUSE",
                reason_code: "ERR_MANUAL_APPROVAL_REQUIRED"
            };
        }
        ...
        return {
            decision: "REFUSE",
            reason_code: "ERR_PARSE"
        };
    }
}
function categoryForMalformed(caseId) {
...
function buildFailureModeReport(validReceipt, secondReceipt) {
    const goodLine = canonicalizeJson(validReceipt);
    const secondLine = canonicalizeJson(secondReceipt);
    ...
    const corruptedReceipt = JSON.parse(goodLine);
    corruptedReceipt.signature.value = `ff${corruptedReceipt.signature.value.slice(2)}`;
    writeFileSync(corruptedPath, `${canonicalizeJson(corruptedReceipt)}\n`, "utf8");
    ...
    const clockBase = rawJson(makeBaseInput());
...
function buildCostImpactReport() {
            ...
            input: makeBaseInput({
                execution_request: {
                    ...
                    runtime_observation: {
                        ...
                        actual_gpu_count: 64,
                        actual_hours: 24,
                        actual_total_cost_cents: 768000
                    }
                }
            })
        },
    ...
    const cases = workloads.map((workload)=>{
        const result = executeOutcome(rawJson(workload.input));
        const projectedCost = workload.input.execution_request.resources.gpu_count * workload.input.execution_request.resources.hours * workload.input.pricing_data.gpu_hour_cents * (workload.input.execution_request.execution.auto_scale ? workload.input.execution_request.execution.max_scale_multiplier : 1) * (workload.input.execution_request.execution.retry_on_fail ? workload.input.execution_request.execution.max_retries + 1 : 1);
...
function buildBenchmarkReportMarkdown(summary, performance) {
    const lines = [
        "# benchmark_report",
        "",
    ...
    for (const profile of PROFILE_ORDER){
        const item = performance.profiles.find((entry)=>entry.profile === profile);
        lines.push(`| ${profile} | ${item.sut.throughput_rps} | ${item.sut.latency_ms.p50} | ${item.sut.latency_ms.p95} | ${item.sut.latency_ms.p99} | ${item.sut.cpu_usage.normalized_percent} | ${item.sut.memory_usage.peak_rss_bytes} | ${item.delta_percent.throughput_rps} | ${item.delta_percent.p95_latency_ms} |`);
    }
    return `${lines.join("\n")}\n`;
}
function main() {
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "test_matrix.json"), buildMatrixReport(allCases));
    const mismatchLogs = [];
    const baselineReceipts = [];
    let determinismRuns = 0;
    let determinismMismatches = 0;
    const profileMismatchCounts = {};
    for (const profile of PROFILE_ORDER){
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "determinism_proof.json"), determinismReport);
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "schema_enforcement_report.json"), {
        schema_version: "ecs.audit.schema_enforcement_report.v1",
        total_invalid_inputs: invalidCases.length,
        rejection_rate: percent(Object.values(categoryStats).reduce((sum, item)=>sum + item.rejected, 0), invalidCases.length),
    ...
    for (const baseCase of canonicalizationBases){
        ...
        const baselineCanonical = canonicalBytes(JSON.parse(baseCase.raw_input));
        for(let variantIndex = 0; variantIndex < 50; variantIndex += 1){
            ...
            const variantCanonical = canonicalBytes(JSON.parse(rawVariant));
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "canonicalization_attack_report.json"), {
    ...
    writeJsonArtifact(join(PROOF_BUNDLE_DIR, "replay_results.json"), replayReport);
    ...
    const rustOutputs = JSON.parse(readFileSync(rustOutputPath, "utf8"));
    const parityVectors = JSON.parse(readFileSync(PARITY_VECTOR_PATH, "utf8"));
    ...
    const parityCases = parityVectors.map((vector, index)=>{
        ...
        const nodeReceipt = nodeOutcome.receipt ? JSON.parse(nodeOutcome.output_bytes) : null;
        const rustReceipt = JSON.parse(rustOutcome.receipt_bytes);
        const caseResult = {
    ...
    writeJsonArtifact(join(PROOF_BUNDLE_DIR, "parity_report.json"), parityReport);
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "performance_report.json"), performanceReport);
    const firstReceipt = baselineReceipts[0];
    const secondReceipt = baselineReceipts[1] ?? baselineReceipts[0];
    const failureModeReport = firstReceipt && secondReceipt ? buildFailureModeReport(firstReceipt, secondReceipt) : {
        schema_version: "ecs.audit.failure_mode_report.v1",
        cases: []
    };
    writeJsonArtifact(join(OUTPUT_DIR, "failure_mode_report.json"), failureModeReport);
    const costImpactReport = buildCostImpactReport();
    writeJsonArtifact(join(OUTPUT_DIR, "cost_impact_report.json"), costImpactReport);
    const summary = {
        schema_version: "ecs.audit.summary.v2",
        generated_at: BENCHMARK_TIMESTAMP,
        total_runs: determinismRuns + canonicalizationRuns + invalidCases.length + replayStore.total + parityCases.length + PERFORMANCE_RUNS_PER_PROFILE * PROFILE_ORDER.length * 2,
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "summary.json"), summary);
    ...
    writeJsonArtifact(join(FULL_LOGS_DIR, "minimal_repro_inputs.json"), {
        determinism: mismatchLogs.slice(0, 5).map((item)=>({
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "manifest.json"), {
...
function VALID_PROFILE_RUNS(profile) {
    return buildProfileCaseCount(profile) * DETERMINISM_REPETITIONS;
}
function buildProfileCaseCount(profile) {
