...
function prepareDirs() {
    ...
    mkdirSync(FULL_LOGS_DIR, {
        recursive: true
    });
    mkdirSync(PROOF_BUNDLE_DIR, {
        recursive: true
    });
}
...
function stableDecisionDistribution() {
    return {
        ALLOW: 0,
        REFUSE: 0
    };
}
function projectedCostCents(rawInput) {
...
function buildHighCostAllowCase(index) {
    ...
    const input = makeBaseInput({
        execution_request: {
            ...
            release_request: {
                ...
                execution_id: `exec-high-cost-${index.toString().padStart(4, "0")}`
            },
            runtime_observation: {
...
function buildRunawayCase(index) {
    ...
    const input = makeBaseInput({
        execution_request: {
            ...
            release_request: {
                hold_state: "PENDING",
                already_consumed: false,
                execution_id: `exec-runaway-${index.toString().padStart(4, "0")}`
            },
            runtime_observation: {
                kill_switch_active: false,
                actual_gpu_count: gpuCount,
    ...
    const input = makeBaseInput({
        execution_request: {
            ...
            release_request: {
                hold_state: "APPROVED",
                already_consumed: false,
                execution_id: `exec-mixed-runtime-${index.toString().padStart(4, "0")}`
            },
            runtime_observation: {
...
function buildScenarios() {
    const highCost = Array.from({
        length: 80
    }, (_, index)=>buildHighCostAllowCase(index));
    const runaway = Array.from({
        length: 80
    }, (_, index)=>buildRunawayCase(index));
    const mixed = Array.from({
        length: 120
    }, (_, index)=>buildMixedCase(index));
    return {
...
function executeEnabled(rawInput) {
...
function executeDisabled(rawInput, scenario) {
    const requestHash = sha256Hex(canonicalizeJson(JSON.parse(rawInput)));
    ...
    const payload = {
        ...
        decision_output: {
            decision: "ALLOW",
            decision_hash: sha256Hex(canonicalizeJson({
                mode: "mnde_disabled",
                scenario,
                request_hash: requestHash,
                decision: "ALLOW",
    ...
    const signature = {
        ...
        value: hmacHex(DISABLED_SIGNING_SECRET, canonicalizeJson(payload))
    };
    const receipt = {
        ...payload,
        signature
...
function buildScenarioReport(scenario, cases) {
    const enabledLatencies = [];
    const disabledLatencies = [];
    ...
    const enabledReceiptLines = [];
    const disabledReceiptLines = [];
    const parityVectors = [];
...
function verifyScenarioReplay(scenario, cases) {
    let driftCount = 0;
    let totalReplays = 0;
    const failureLogs = [];
    for (const testCase of cases){
...
function buildFaultCases() {
    ...
    const gpuViolation = rawJson(makeBaseInput({
        execution_request: {
    ...
    const retryViolation = rawJson(makeBaseInput({
        execution_request: {
    ...
    return [
        {
            case_id: "fault-malformed",
            category: "malformed_json",
            expected_reason_code: "ERR_INVALID_JSON_SYNTAX",
            raw_input: "{\"execution_request\":"
        },
        {
            case_id: "fault-duplicate",
            category: "duplicate_keys",
            expected_reason_code: "ERR_DUPLICATE_JSON_KEYS",
            raw_input: duplicateKeys
        },
        {
            case_id: "fault-unknown",
            category: "unknown_fields",
            expected_reason_code: "ERR_SCHEMA_VALIDATION",
            raw_input: unknownField
        },
        {
            case_id: "fault-boundary-gpu",
            category: "boundary_limit_violation",
            expected_reason_code: "ERR_GPU_LIMIT",
            raw_input: gpuViolation
        },
        {
            case_id: "fault-boundary-retry",
            category: "boundary_limit_violation",
            expected_reason_code: "ERR_RETRY_LIMIT",
            raw_input: retryViolation
        }
    ];
}
function evaluateFaultClassification(cases) {
...
function runParity(vectors) {
    ...
    const rustReceipts = JSON.parse(rustOutput);
    const parityResults = vectors.map((vector, index)=>{
        ...
        const rustSignedReceipt = JSON.parse(rustReceipt.receipt_bytes);
        return {
...
function runContinuousLoadSample(scenarios) {
    const combined = [
        ...scenarios.high_cost_allow_path,
        ...scenarios.runaway_execution_path,
        ...scenarios.mixed_production_traffic
    ];
    const latencies = [];
...
function main() {
    ...
    const parityVectors = [];
    const replayReports = [];
    for (const [scenario, cases] of Object.entries(scenarios)){
    ...
    writeJsonArtifact(join(OUTPUT_DIR, "final_report.json"), finalReport);
    writeJsonArtifact(join(OUTPUT_DIR, "scenario_reports.json"), scenarioReports);
    writeJsonArtifact(join(OUTPUT_DIR, "replay_report.json"), replayReports);
    writeJsonArtifact(join(OUTPUT_DIR, "parity_report.json"), parityReport);
    writeJsonArtifact(join(OUTPUT_DIR, "fault_injection_report.json"), faultReport);
    writeJsonArtifact(join(OUTPUT_DIR, "continuous_load_report.json"), continuousLoadReport);
