...
import { REASON_CODES, canonicalizeJson, canonicalPolicyPayload, deriveKeyId, parseStrictJson, policyHash, verifyPolicySignature } from "../shared/index.js";
...
function decisionHashFromFailure(requestHash, reasonCode) {
...
function typedFailure(requestHash, reasonCode) {
    return {
        decision: "REFUSE",
        request_hash: requestHash,
...
function rejectUnknownKeys(root, allowedKeys) {
...
function validateInteger(value, minimum, allowZero) {
    if (typeof value !== "number") {
        return {
            ok: false,
            reason_code: REASON_CODES.TypeMismatch
        };
    }
    if (Object.is(value, -0)) {
        return {
            ok: false,
            reason_code: REASON_CODES.InvalidJsonNumber
        };
    }
...
function parseToolCalls(value) {
    if (!Array.isArray(value) || value.length === 0) {
        return {
            ok: false,
            reason_code: REASON_CODES.SchemaValidation
        };
    }
    const parsed = [];
    for (const item of value){
        ...
        const unknown = rejectUnknownKeys(item, [
            "tool",
            "priority"
        ]);
        if (unknown) {
        ...
        if (!tool || !priority.ok) {
            return {
                ok: false,
                reason_code: tool ? priority.reason_code : REASON_CODES.TypeMismatch
            };
        }
        parsed.push({
            tool,
...
function parseParameters(value) {
    ...
    for (const key of Object.keys(value)){
        if (RESERVED_UNSAFE_FIELDS.has(key)) {
            return {
                ok: false,
                reason_code: REASON_CODES.NonDeterministicInput
            };
        }
    }
    return {
...
export function runStrictPreflight(rawInput) {
    const parsed = parseStrictJson(rawInput);
    if (!parsed.ok) {
        const requestHash = createHash("sha256").update(rawInput).digest("hex");
        const reasonCode = parsed.reason === "duplicate_json_keys" ? REASON_CODES.DuplicateJsonKeys : parsed.reason === "invalid_json_number" ? REASON_CODES.InvalidJsonNumber : REASON_CODES.InvalidJsonSyntax;
    ...
    const canonicalInput = canonicalizeJson(parsed.value);
    const requestHash = createHash("sha256").update(canonicalInput).digest("hex");
    const root = parsed.value;
    const rootUnknown = rejectUnknownKeys(root, [
        "execution_request",
        "policy_document",
        "pricing_data"
    ]);
    ...
    const executionUnknown = rejectUnknownKeys(executionRequest, [
        "request_id",
        "submitted_region",
        "actor",
        "resources",
    ...
    const actorUnknown = rejectUnknownKeys(actor, [
        "user_id"
    ]);
    const resourcesUnknown = rejectUnknownKeys(resources, [
        "gpu_type",
        "gpu_count",
        "hours"
    ]);
    const executionConfigUnknown = rejectUnknownKeys(execution, [
        "auto_scale",
        "max_scale_multiplier",
        "retry_on_fail",
        "max_retries"
    ]);
    const orbitUnknown = rejectUnknownKeys(orbitIntent, [
        "orbit_version",
        "action",
        "boundary",
        "payload",
    ...
    const releaseUnknown = rejectUnknownKeys(releaseRequest, [
        "execution_id",
        "hold_state",
        "already_consumed"
    ]);
    const runtimeUnknown = rejectUnknownKeys(runtimeObservation, [
        "kill_switch_active",
        "actual_gpu_count",
        "actual_hours",
        "actual_total_cost_cents"
    ]);
    const pricingUnknown = rejectUnknownKeys(pricingData, [
        "gpu_hour_cents"
    ]);
    if (actorUnknown || resourcesUnknown || executionConfigUnknown || orbitUnknown || releaseUnknown || runtimeUnknown || pricingUnknown) {
        return typedFailure(requestHash, actorUnknown ?? resourcesUnknown ?? executionConfigUnknown ?? orbitUnknown ?? releaseUnknown ?? runtimeUnknown ?? pricingUnknown ?? REASON_CODES.SchemaValidation);
    }
    const policyUnknown = rejectUnknownKeys(policyDocument, [
        "schema_version",
        "policy_version",
        "rules",
        "trust"
    ...
    for (const signature of orbitIntent.signatures){
        ...
        const unknown = rejectUnknownKeys(signature, [
            "alg",
            "sig"
        ]);
        if (unknown) {
    ...
    const orbitPayloadUnknown = rejectUnknownKeys(orbitPayload, [
        "tool_calls"
    ]);
    ...
    if (policyDocument.trust !== undefined) {
        ...
        const trustUnknown = rejectUnknownKeys(policyDocument.trust, [
            "key_version",
            "key_id",
            "public_key",
            "signature"
    ...
    const built = {
        ...
        policy_document: {
            ...
            ...policyDocument.trust === undefined ? {} : {
                trust: {
                    key_version: policyDocument.trust.key_version,
                    key_id: policyDocument.trust.key_id,
                    public_key: policyDocument.trust.public_key,
                    signature: policyDocument.trust.signature
                }
            },
            rules: {
                max_total_cost_cents: maxTotalCostCents.value,
