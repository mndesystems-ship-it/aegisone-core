...
import { canonicalizeJson, hashCanonicalJson, sha256Hex } from "../shared/index.js";
...
function keyIdFromPublicKey(publicKeyHex) {
    return sha256Hex(Buffer.from(publicKeyHex, "hex")).slice(0, 16);
}
export function verifyPolicyTrust(policyObject, requestObject, pinnedPolicyVersion) {
    const reasons = [];
    const policyPayload = buildPolicyPayload(policyObject);
    const policyHash = hashCanonicalJson(policyPayload);
    ...
    const allowedKeyUniverse = new Set(ALLOWED_REQUEST_KEYS);
    ...
    try {
        const ok = verify(null, Buffer.from(canonicalizeJson(policyPayload), "utf8"), toPublicKeyObject(policyObject.trust.signing_public_key), Buffer.from(policyObject.trust.signature, "hex"));
        if (!ok) {
