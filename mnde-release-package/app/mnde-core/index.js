import { evaluateReleaseControl } from "../arm/index.js";
import { validateOrbitIntent } from "../orbit/index.js";
import { verifyPolicyTrust } from "../policy/index.js";
import { runPreflight } from "../preflight/index.js";
import { evaluateRuntimeRefusal } from "../ramona/index.js";
...
export function runMndePipeline(rawInput, pinnedPolicyVersion = "policy.v1") {
    ...
    const policyTrustResult = verifyPolicyTrust(preflightResult.policy_object, preflightResult.request_object, pinnedPolicyVersion);
    preflightResult.request_object.runtime_request.observed_policy_hash = policyTrustResult.policy_hash;
    const armResult = evaluateReleaseControl(preflightResult.request_object, orbitResult, policyTrustResult, preflightResult.policy_object.rules);
    if (orbitResult.decision !== "PASS" || !policyTrustResult.trusted || armResult.decision !== "ALLOW") {
        const upstreamDecision = orbitResult.decision !== "PASS" || !policyTrustResult.trusted ? "REFUSE" : armResult.decision;
        return {
            ...
            decision_object: buildDecisionObject({
                ...
                reasons: [
                    ...orbitResult.decision === "PASS" ? [] : [
                        `orbit.reason=${orbitResult.reason}`
                    ],
    ...
    const ramonaResult = evaluateRuntimeRefusal(preflightResult.request_object, preflightResult.request_hash, policyTrustResult.policy_hash, armResult.projected_total_cost_usd, preflightResult.policy_object.rules.max_total_cost_usd);
    return {
        request_object: preflightResult.request_object,
        policy_object: preflightResult.policy_object,
        orbit_result: orbitResult,
