...
function generateCostRequests(count) {
    const requests = [];
...
function generateAgentRuns(count) {
    const runs = [];
    for(let index = 0; index < count; index += 1){
...
function simpleCostForRequest(request) {
...
function evaluateStandardCostRequest(request, index) {
    const reasons = [];
...
function evaluateStandardAgentRun(run, index) {
    const reasons = [];
    for (const action of run.actions){
...
function workloadHash(costRequests, agentRuns) {
    ...
    const combined = {
        ...
        validation_checks: {
            ...
            refusal_categories_align_with_known_patterns: Object.keys(refusalCategories).every((key)=>[
                    "MAX_RETRY_LIMIT",
                    "GLOBAL_TIMEOUT_CAP",
                    "SIMPLE_BUDGET_CEILING",
                    "BASIC_RATE_LIMIT"
                ].includes(key)),
            no_negative_cost_values: [
                noControlCost,
                totalCost,
                existingCost.total_cost_with_mnde_micro_usd,
                noControlAgentCost,
                agentCost,
                existingAgent.cost_generated_with_mnde_micro_usd
            ].every((value)=>value >= 0),
